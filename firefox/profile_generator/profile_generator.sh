...
reGenerateProfile(){
    ...
    FIREFOX_HOME_DIR="$HOME/.mozilla/firefox"
    ...
    # append body title
    TITLE_NEW='PROFILE: '$PROFILE_NAME
    echo $TITLE_NEW >> "$HOME_PAGE_FILE_PATH" || exit 106

    # append config buttons
    cat "$BOOKMARKS_TEMPLATE_DIR"_3.html >> "$HOME_PAGE_FILE_PATH" || exit 104

    # append the links (last part of HTML)
...
# EXTENSIONS_XPI_DIR="$HOME/.mozilla/firefox/linkedin/extensions"
# 
